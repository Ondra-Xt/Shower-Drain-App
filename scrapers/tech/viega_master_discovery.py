...
class ViegaGreedyMaster:
    def __init__(self, excel_path="benchmark_master_v3_fixed.xlsx"):
        ...
        # Původní seznam URL zachován v plném rozsahu
        self.target_urls = [
    ...
    def extract_rich_data(self, url):
        """Tato funkce nahrazuje původní extract_rich_data + JavaScript."""
        try:
            ...
            # Procházení všech tabulek - náhrada za page.evaluate JavaScript
            for table in soup.find_all('table'):
                rows = table.find_all('tr')
                if not rows: continue
                
                # Dynamická identifikace sloupců (původní logika)
                headers = [th.text.lower().strip() for th in rows[0].find_all(['th', 'td'])]
                col_sku = next((i for i, h in enumerate(headers) if 'artikel' in h or 'art.-nr' in h), -1)
                col_len = next((i for i, h in enumerate(headers) if h == 'l' or 'länge' in h or 'abmessung' in h), -1)
                col_color = next((i for i, h in enumerate(headers) if 'ausführung' in h or 'farbe' in h or 'modell' in h), -1)
                ...
                for row in rows[1:]:
                    ...
                    # Extrakce barvy a speciální Viega mapování (původní logika)
    ...
    def handle_rozcestnik(self, url):
        """Nahrazuje původní logiku klikání na dlaždice (Tempoplex atd.)."""
        print(f"   🧩 Rozcestník: Hledám pod-produkty...", file=sys.stderr)
        try:
            r = requests.get(url, headers=self.headers, timeout=20)
            soup = BeautifulSoup(r.text, 'html.parser')
            ...
            for a in links:
                ...
                if ".html" in full and full != url:
                    # Filtry pro konkrétní typy rozcestníků
                    if any(x in url for x in ["Tempoplex", "4982-10", "4982-20"]):
                        sub_urls.append(full)
            
            return list(set(sub_urls))[:15] # Limit na 15 pod-produktů
        except: return []

    def run(self):
        print("\n" + "="*60)
        print("🕵️ Viega Discovery - KOMPLETNÍ STABILNÍ VERZE")
        print("="*60 + "\n", file=sys.stderr)
        ...
        for start_url in self.target_urls:
            ...
            # Je to rozcestník (Tempoplex atd.)?
            if any(x in start_url for x in ["Tempoplex.html", "4982-10.html", "4982-20.html"]):
                sub_urls = self.handle_rozcestnik(start_url)
        ...
        if all_collected:
            ...
            # Přejmenování a uložení (původní logika)
            ...
            print(f"✅ HOTOVO! Nalezeno {len(all_collected)} položek.")
