...
class ViegaPricingBotProtected:
    ...
    def run(self):
        ...
        print("\n" + "="*60)
        print("🚀 Spouštím KROK 2: Viega Pricing Bot (CHYTRÉ OŘÍZNUTÍ STRÁNKY)")
        print("="*60 + "\n", file=sys.stderr)
            ...
            for target_sku in skus_to_search:
                ...
                if self.search_megabad(page, target_sku):
                    ...
                    try:
                        ...
                        clean_sku = re.sub(r'[\s.\-]', '', target_sku.lower())
                        
                        if clean_sku not in clean_html:
                            print(f"      ⚠️ Přeskakuji: Kód {target_sku} na cílové stránce opravdu vůbec není.", file=sys.stderr)
                            continue

                        # --- NOVÁ LOGIKA: OŘÍZNUTÍ TEXTU PŘED DOPORUČENÝMI PRODUKTY ---
                        price, uvp = "", ""
                        
                        # Zkusíme nejprve meta tagy
                        try:
                        ...
                        # Pokud meta tag selže, hledáme v oříznutém textu
                        if not price:
                            # Usekneme text, jakmile začne sekce s dalšími produkty
                            ...
                            for word in stop_words:
                                idx = search_text.lower().find(word)
                                if idx != -1:
                                    search_text = search_text[:idx] # Ořízneme!
                            
                            # Abychom to ještě zrychlili, vezmeme jen prvních 1500 znaků (tam cena musí být)
                            search_text = search_text[:1500].replace('\xa0', ' ')
                        ...
                        # Extrakce chybějících parametrů
                        length, is_cuttable = "", ""
